...
const FrontPage = (() => {
	...
	return class FrontPage extends Component {
		state = {
			tableRows: [],
			isHoliday:
				GM_getValue('isHoliday') ?? (GM_setValue('isHoliday', false), false),
			isEmpty: false,
			type: undefined
		};
		...
		timeout = {
			...
			setCoursesTimeout: delay => {
				this.timeout.clearTimeout();
				this.timeout._t = setTimeout(this.updateCourse.bind(this), delay, true);
			}
		};
		...
		render = () => {
			const {isEmpty, isHoliday, type, tableRows} = this.state;
