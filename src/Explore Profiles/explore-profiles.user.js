				...
				<div class="epr-notification">
					<div class="epr-centered">
						...
						<div class="epr-text-center">
							{`Checking ${to === undefined ? from : Math.min(to, from)}`}
							{to !== undefined && ` to ${Math.max(to, from)}`}
						</div>
					</div>
				</div>
			)
...
class Sidebar extends Component {
	...
	render = () => {
		const {isUserProfile, id, fullname} = this.state;
...
const fetchNewProfile = async event_ => {
	...
	const id = Number(new URL(location).searchParams.get('id'));
	const action = Number(target?.dataset?.action ?? 0);
	let profile;
	...
	if (Number.isFinite(action)) {
		...
		if (!profile) {
			profile = profiles[action < 0 ? profiles.length - 1 : 0];
		}
	} else {
		let profiles = [];
...
const runOnce = () => {
	...
	if (GM_getValue('highest') === undefined) {
		GM_setValue(
			'highest',
			1954 // Highest + 10 at time of creation
