...
const getPageContent = async ( noCache = false ) => login( noCache ).then( async ( wstoken ): Promise<false | Array<GetContentsResponse>> => {
  ...
  return fetch(
      ...
      .then( async ( response ): Promise<Array<GetContentsResponse> | GetContentsResponseFailed> => response.json() )
      .then( ( responseJSON ) => {
...
const generateImageURL = ( () => {
  ...
  return (
      mimetype: string, defaultValue: string,
  ) => {
...
const Folder = ( {
  ...
  contents: Array<GetContentsModuleContentsIsFileSanitized>;
  base?: string;
  directoryDepth?: number;
} ) => {
  const filePaths: Record<
  string,
  Array<GetContentsModuleContentsIsFileSanitized>
  > = {
    '/': [],
  };
...
const handleClick = ( () => {
  ...
  return ( event_: MouseEvent ) => {
    ...
    const subFolder = event_.target.closest<HTMLDivElement>( 'div.fp-filename-icon' );

    if ( subFolder ) {
      const subFolderContent = subFolder.nextElementSibling as HTMLUListElement;

      subFolderContent.hidden = !subFolderContent.hidden;
      const caretIcon = subFolder.querySelector<HTMLElement>( '.folders-inline-caret' );
      if ( caretIcon ) {
    ...
    if ( anchor?.pathname === '/mod/folder/view.php' && folder ) {
      ...
      void pageContent.then( ( pageContentJSON ) => {
        ...
        const sanitizedContents: Array<GetContentsModuleContentsIsFileSanitized> = [];
        ...
        render(
            <Folder contents={sanitizedContents} />,
            frag,
        );
