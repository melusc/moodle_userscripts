    ...
    if (anchor?.pathname === '/mod/folder/view.php') {
      ...
      login().then(token => {
        ...
        pageContent.then(pageContentJSON => {
          ...
          const sectionId = +section.getAttribute('aria-labelledby').match(/(?<=-)\d+(?=-)/)[0];
          const sectionObj = pageContentJSON.find(({
            id
          }) => id === sectionId);
          const {
            modules
          ...
          for (let i = 0; i < contents.length; i++) {
            ...
            contents[i].filepath = contents[i].filepath.split('/').filter(e => e !== '');
          }

          const frag = document.createDocumentFragment();
          render(h(GenerateFolder, {
            contents: contents
          }), frag);
          folder.append(frag);
        });
...
const generateImageURL = (() => {
  const imageURLs = {
    ...
    'application/vnd.openxmlformats-officedocument.wordprocessingml.document': 'document-256',
    'application/msword': 'document-256',
    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': 'spreadsheet-256',
    'application/vnd.ms-excel': 'spreadsheet-256',
    'application/vnd.openxmlformats-officedocument.presentationml.presentation': 'powerpoint-256',
