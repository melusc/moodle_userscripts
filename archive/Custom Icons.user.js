...
// @Author      lusc
// @downloadURL https://raw.githubusercontent.com/melusc/lusc/master/archive/Custom%20Icons.user.js
// @updateURL   https://raw.githubusercontent.com/melusc/lusc/master/archive/Custom%20Icons.user.js
// @grant       GM_getValue
// @grant       GM_setValue
  ...
  if (remote) {
    await fetch('/')
      ...
      .then(e => {
        ...
        document.getElementById('inst4').innerHTML = e.getElementById(
          'inst4'
        ).innerHTML;
      });
    dispatchEvent(
      new CustomEvent('cleanMoodle', { detail: { newPage: false } })
    );
...
addEventListener('DOMContentLoaded', () => {
  if (location.pathname === '/user/preferences.php') {
    dispatchEvent(new Event('customIcons'));
    const row = document.getElementById('maincontent').parentElement
        .children[2],
...
if (location.pathname.toLowerCase().startsWith('/customicons')) {
  ...
  fetch('/')
    ...
    .then(e => {
      ...
      document.body
        .getElementsByClassName('type_setting depth_2 item_with_icon')[0]
        .remove();

      const regionMainBox = parsed.getElementById('region-main-box'),
        regionMainBoxLi = regionMainBox.getElementsByClassName(
          'section img-text'
        )[0];
      ...
      document
        ...
        .insertBefore(
          regionMainBox,
          document.getElementById('page-content').children[0]
        );
      document.getElementById('user-notifications').remove();
      dispatchEvent(
        new CustomEvent('cleanMoodle', { detail: { newPage: true } })
      );
      dispatchEvent(new Event('customIcons'));
      ...
      const selectedCourseDiv = document.createElement('div'),
        selectedCourseText = document.createTextNode(
          'Select course on the left'
        );
...
async function handleFile(e) {
  if (e.key === 'Enter' || e.key === undefined) {
    ...
    if (
      !(
        (fileInput.files[0] || urlInput.value) &&
