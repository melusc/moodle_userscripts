...
function sort(e, sidebar) {
    ...
    if (sidebar) {
        ...
        console.log('Clean moodle version %c' + GM_info.script.version + ' %cby %clusc', 'color: #fe4c4c', 'color:', 'color:#58e');
    }
}
    ...
    fetch('https://moodle.ksasz.ch/')
        ...
        .then(e => {
            ...
            if (login) {
                ...
                const aside = response.getElementById('block-region-side-pre').cloneNode(false);
                aside.appendChild(sidebar);

                const section = response.querySelector('section.hidden-print').cloneNode(false);
                ...
                const div2 = response.getElementById('page-content').cloneNode(false);
                div2.appendChild(div5);
                div2.appendChild(div);

                const div3 = response.getElementById('page').cloneNode(false);
                div3.appendChild(div2);

                const div4 = response.getElementById('page-wrapper').cloneNode(false);
