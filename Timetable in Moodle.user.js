...
// ==/UserScript==
/* jshint esversion: 10 */
/* global updateTextarea: true, textarea: true, handleJSON: true, validateTime: true */
'use strict';
...
if (
  location.pathname.indexOf('Stundenplan') !== -1 &&
  document.getElementById('userScriptSave')
) {
  const storage = {};
  ...
  sidebar = document
    .querySelector('li.type_unknown.depth_1.contains_branch.current_branch')
...
function time(dontNotify) {
  ...
  if (day === 0 || day === 6) {
    if (innerDiv.textContent.indexOf('Wochenende') === -1) {
...
function createRow(name, time, value) {
  ...
  if (value !== 'Frei') {
    for (let i = 0; i < sidebar.childElementCount; i++) {
      if (
        sidebar.children[i].nodeName === 'LI' &&
        sidebar.children[i]
...
function parseTime(e) {
  if (typeof e !== 'undefined') {
    ...
    const currentTime = `${('0' + firstHour).slice(-2)}:${(
      '0' + firstMin
    ).slice(-2)} - ${('0' + secondHour).slice(-2)}:${('0' + secondMin).slice(
      -2
    )}`;
    return currentTime;
  } else return null;
